...
const initializeSerialPort = ({ SERIAL_PORT_NAME, SERIAL_PORT_CONFIGURATION }, serialPortSentenceHandler) => {
    const cycle = () => {
        ...
        setTimeout(() => serialPortSentenceHandler(`GPS 151.5 -33.0 10 180 8 0 ${getGpsDate()}`), 200);
        ...
        setTimeout(() => serialPortSentenceHandler("RPM 2400"), 600);
        setTimeout(() => serialPortSentenceHandler(`GPS 151.5 -33.0 20 180 8 0 ${getGpsDate()}`), 600);
        setTimeout(() => serialPortSentenceHandler("RDG 14.25 33.5 55 90"), 800);
        setTimeout(() => serialPortSentenceHandler("RPM 4100"), 1000);
        setTimeout(() => serialPortSentenceHandler(`GPS 151.5 -33.0 70 180 8 0 ${getGpsDate()}`), 1000);
        ...
        setTimeout(() => serialPortSentenceHandler("RPM 2800"), 1300);
        setTimeout(() => serialPortSentenceHandler(`GPS 151.5 -33.0 30 180 8 0 ${getGpsDate()}`), 1300);
        ...
        setTimeout(() => serialPortSentenceHandler(`GPS 151.5 -33.0 20 180 8 0 ${getGpsDate()}`), 1800);
        setTimeout(() => serialPortSentenceHandler("RPM 1500"), 1800);
    };
